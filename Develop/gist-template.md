...
In this tutorial we will go over a short tutorial of Regex (regular expressions).

## Summary

Regex is a string of text that allows you to search given text for your Regex string. An example code snippet of regex shows as following:
```
/[\w._%+-]+@[\w.-]+\.[a-zA-z]{2,4}/
```
...
* `*` - checks if a string has zero or more of the previous character
`xyz*        matches a string that has xy followed by zero or more z`
* `+` - checks if a string has one or more of the previous character
`xyz+        matches a string that has xy followed by one or more z`
* `?` - checks if a string has zero or one of the previous character
`xyz?        matches a string that has xy followed by zero or one z`
* `{}` -  checks if a string has however many are inside the brackets of the previous character
```
...
[GitHub Profile](https://github.com/HallerJohn)
